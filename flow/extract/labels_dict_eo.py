...
def query_eo_labels(s, cfs_entries, species_filter, species_mappings):
    # get all relevant cam ids
    cam_ids = [cam.id for cam in cfs_entries]

    # get the fused images with headers from that camera
    images = s.query(EOImage, TrainTestSplit)\
        .join(TrainTestSplit, TrainTestSplit.image_id == EOImage.file_name)\
        .join(HeaderMeta)\
        .filter(HeaderMeta.camera_id.in_(cam_ids)).all()


    # get labels for the eo image of the fused image
    eo_image_ids = [f.file_name for f,tts in images]
...
def eo_labels_dict(filter_path):
    with mlflow.start_run(run_name='eo_labels_dict', experiment_id=experiment.experiment_id) as mlrun:
        mlflow.set_tag('method_fusion', False)
        ...
        train_dict, test_dict = query_eo_labels(s, cfs_entries, filters['species'], filters['species_mappings'])
...
if __name__ == '__main__':
    eo_labels_dict('/home/yuval/Documents/XNOR/sealnet-mlflow/flow/extract/labels_dict_filters.json')
